...
机器翻译与 LLM 各自有独立的「一次窗口大小 / 最多上文 / 单次最大窗口字符数量」配置。字符上限默认分别为 `4500` 和 `1500`：达到上限时优先保留本次待翻译正文，并用剩余额度自动缩减上文。LLM 默认窗口 30、上文 5；请求 LLM 时还会按「输出上限倍数」（默认 3）设置 `max_tokens = 输入字符数 × 倍数`。
